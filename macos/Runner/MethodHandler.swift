    ...
    private func handleInit(result: @escaping FlutterResult) {
        if VPNManager.useSystemExtension {
            do {
                ...
                for path in [FilePath.sharedBasePath, FilePath.sharedTempPath] {
        ...
        if VPNManager.useSystemExtension {
            paths = [
                "basePath": FilePath.sharedBasePath,
                "workingPath": FilePath.sharedWorkingPath,
                "tempPath": FilePath.sharedTempPath
            ]
        } else {
            paths = [
...
//            let resultConfig = LibboxGetFullConfig(config, nil)
//            print(resultConfig)
            ...
            do {
                ...
                let errorType = getVPNErrorType(vpnError)
                let darwinMajor = ProcessInfo.processInfo.operatingSystemVersion.majorVersion
                await mainResult(FlutterError(code: "VPN_ERROR",
                                              message: vpnError.localizedDescription,
                                              details: [
                                                "errorType": errorType,
                                                "macOSVersionMajor": darwinMajor,
                                              ]))
                return
            } catch {
    ...
    private func getVPNErrorType(_ error: VPNError) -> String {
        switch error {
        case .systemExtensionRequiresUserApproval:
            return "systemExtensionRequiresUserApproval"
        case .systemExtensionUnknownError:
            return "systemExtensionUnknownError"
        }
    }
}
